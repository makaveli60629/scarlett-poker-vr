...
export function setupWorld(scene, camera) {
  ...
  // Floating art
  loader.load('assets/textures/casino_art.jpg', tex=>{
    const artMat = new THREE.MeshStandardMaterial({ map:tex });
    const artPlane = new THREE.Mesh(new THREE.PlaneGeometry(3,2), artMat);
    artPlane.position.set(-5,2,-5); scene.add(artPlane);
  });
}
