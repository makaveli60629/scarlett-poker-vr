// js/main.js — Patch 6.4 FULL
// Adds: Shop UI + Inventory + Avatar Preview + Menu Toggle + Grip Action hook
// Keeps: Patch 6.3 SolidWalls + Collision + SpawnRescue
...
export async function boot() {
  ...
  // Boss zone (spectator only + VIP rail)
  ...
  // Patch 6.4: Input + Inventory + UI + Avatar Preview
  Input.init(renderer);
  Inventory.init();
  ...
  overlay("Loaded ✅\nPatch 6.4: Shop + Inventory + Avatar Preview\nMenu/M opens Store");
  ...
  renderer.setAnimationLoop(() => {
    ...
    // Update input first (menu/grip)
    Input.update();

    // Menu toggle
    if (Input.menuPressed()) {
      ShopUI.toggle();
      toast(ShopUI.open ? "Store opened" : "Store closed");
    }

    // Grip action hook (next patch we’ll raycast to kiosk/chip reliably)
    if (Input.gripPressed()) {
      // For now, just confirm the grip works (prevents confusion)
      toast("Grip action detected");
    }
    ...
    // Collision + rescue to teleport pad
