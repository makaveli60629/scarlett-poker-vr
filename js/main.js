...
/* ---------- Quest input (correct mapping) ---------- */
function getXRInputs(renderer){
  const out = {
    left: {x:0,y:0, buttons:[]},
    right:{x:0,y:0, buttons:[]},
    has:false
  };
  ...
  for(const src of session.inputSources){
    ...
    if(hand === 'left'){
      out.left.x = x; out.left.y = y; out.left.buttons = btns; out.has=true;
    } else if(hand === 'right'){
      out.right.x = x; out.right.y = y; out.right.buttons = btns; out.has=true;
    } else {
      // fallback: if handedness missing, assign first seen to left then right
      if(!out.has){
        out.left.x=x; out.left.y=y; out.left.buttons=btns; out.has=true;
      } else {
        out.right.x=x; out.right.y=y; out.right.buttons=btns; out.has=true;
...
export async function boot(){
  installCrashHooks();
  setStatus('booting…');
  logLine('✅ boot() starting — controls restructure');
  ...
  // Chairs (not black anymore)
  const chairTex=leatherTex();
  const chairM=new THREE.MeshStandardMaterial({
    color:0x50586f, roughness:0.92, metalness:0.04, map:chairTex
  });
  function buildChair(){
    const g=new THREE.Group();
  ...
  // Audio (toggle required at least once)
  ...
  audioLoader.load(
    ...
    (buf)=>{ music.setBuffer(buf); music.setLoop(true); music.setVolume(0.55); musicReady=true; logLine('🎵 Audio ready (press A/X to toggle)'); },
    undefined,
  ...
  if($('btnMenu')) $('btnMenu').onclick=()=>{ logLine('⌚ Menu (next update: wrist UI)'); };
  ...
  /* ---------- VR controllers: models + rays + teleport ---------- */
  ...
  function calcTeleport(fromController){
    ...
    if(dist > CFG.teleportMax) return false;
  ...
  function teleportTo(point){
    // keep head height, move rig base
    rig.position.x = point.x;
    rig.position.z = point.z;
  }

  function teleportToTable(){
    // a safe spot near table so you can inspect it fast
  ...
  renderer.xr.addEventListener('sessionstart', ()=>{
    ...
    logLine('🎮 Controls: LEFT stick move | RIGHT stick snap turn | RIGHT trigger teleport');
    logLine('🔊 Press A or X to toggle audio | Press B or Y to teleport-to-table');
  });
  ...
  renderer.setAnimationLoop(()=>{
    ...
    if(inXR){
      ...
      // Left stick movement (LEFT controller)
      moveX = deadzone(inputs.left.x);
      moveY = deadzone(inputs.left.y);

      // Right stick snap turn (RIGHT controller)
      turnX = deadzone(inputs.right.x, 0.25);

      // Buttons (safe-ish mapping)
      // buttons[4]/[5] differ by device; we accept either on either controller:
      const lb = inputs.left.buttons;
      const rb = inputs.right.buttons;

      audioBtnNow = pressed(lb?.[4]) || pressed(lb?.[5]) || pressed(rb?.[4]) || pressed(rb?.[5]); // A/X style
      quickBtnNow = pressed(lb?.[3]) || pressed(rb?.[3]); // thumbstick click
      trigNow = pressed(rb?.[0]) || pressed(lb?.[0]); // trigger (prefer right)
    }
    ...
    // Quick teleport-to-table on thumbstick click (works even if you can’t reach)
    if(quickBtnNow && !lastQuickBtn) teleportToTable();
    lastQuickBtn = quickBtnNow;

    // Teleport aiming with trigger (prefer right controller ray)
    if(inXR){
      if(trigNow && !lastTrig){
        teleportArmed = true;
        tpMarker.visible = false;
      ...
      if(trigNow){
        // aim using controller #1 (usually right) but fall back
  ...
  logLine('✅ Restructure applied: correct Quest mapping + teleport + controller models');
  setStatus('running ✅');
}
