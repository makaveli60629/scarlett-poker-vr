// js/world.js
(function(){
  const D = window.SCARLETT_DIAG || { log: (...a)=>console.log(...a) };
  ...
  function buildJumbosAndDoors(){
    // Doors at the 4 cardinal points
    const spots = [
    ...
    spots.forEach((s, idx)=>{
      ...
      // Rank sign UNDER jumbotron, above door
    ...
    // Store displays near STORE door (kept clear of pillars)
  ...
  function buildSpawn(){
    const spawn = el("a-entity",{id:"spawnPad", position:"0 0 28"});
    spawn.appendChild(el("a-cone",{
      id:"spawnArrow",
      radiusBottom:"0.22",
      radiusTop:"0.02",
      height:"0.45",
      position:"0 0.25 -0.85",
      material:"color:#4aa6ff; emissive:#4aa6ff; emissiveIntensity:0.8"
    }));
  ...
  function buildDivotAndTable(){
    // Pit lowered into the ground so you can stand at rail and look down
    const pit = el("a-entity",{id:"pit", position:"0 -0.85 0"});
    ...
    pit.appendChild(el("a-torus",{
      ...
      rotation:"90 0 0",
      position:"0 0.95 0",
      material:"color:#2a1f18; roughness:0.9; metalness:0.08"
    }));
    pit.appendChild(el("a-torus",{
      radius:"7.45",
      radiusTubular:"0.06",
    ...
    // Community frame (raised slightly because table was lowered)
    const comm = el("a-entity",{id:"communityFrame", position:"0 2.05 -1.30"});
    ...
    // small action HUD above community cards (not giant scoreboard)
    ...
    for(let i=0;i<10;i++){
      chipPile.appendChild(el("a-cylinder",{
        radius:"0.06",
        height:"0.02",
    ...
    // Chairs oriented toward table center + SeatAnchors
    for(let i=0;i<6;i++){
  ...
  function buildBots(){
    ...
    for(let i=0;i<6;i++){
      ...
      // hole cards hovering (you can “peek”)
      ...
      // name tag hidden by default (look-to-show module handles it)
  ...
  // 🔒 AUTO-BUILD GUARANTEE (prevents dark screen / “waiting for build”)
  setTimeout(() => {
    if (window.SCARLETT_WORLD && typeof window.SCARLETT_WORLD.build === "function") {
      try {
        window.SCARLETT_WORLD.build();
        if (window.SCARLETT_DIAG) window.SCARLETT_DIAG.log("[world] auto-build executed ✅");
      } catch (e) {
        if (window.SCARLETT_DIAG) window.SCARLETT_DIAG.log("[world] auto-build failed ❌");
        console.error(e);
      }
    }
  }, 0);
