// /js/scarlett1/spine_xr.js — Scarlett XR Spine v2.4 PERMA + Hands (SAFE)
// ✅ KEEP PERMANENT: right stick move + snap turn + trigger teleport (polling)
// ✅ Adds: Controller models + XR Hands (visual only)
// ✅ No left-hand controls yet, no menu
...
export async function installXR({ THREE, DIAG }) {
  ...
  // ✅ Add controller models (Touch controllers)
  try {
    ...
    D.log("[xr] controller models ✅");
  } catch (e) {
    D.warn("[xr] controller models failed (safe)", e?.message || e);
  }

  // ✅ Add real XR hands (Quest hand tracking, visual only)
  try {
    ...
    handL.add(hmf.createHandModel(handL, "mesh"));
    handR.add(hmf.createHandModel(handR, "mesh"));
    rig.add(handL);
    rig.add(handR);

    D.log("[xr] hand models ✅ (if hand-tracking available)");
  } catch (e) {
    D.warn("[xr] hand models failed (safe)", e?.message || e);
  }
  ...
  // ✅ PERMANENT CONTROLS (do not change)
  const DEAD = 0.18;
  ...
  renderer.xr.addEventListener("sessionstart", () => {
    D.log("[xr] sessionstart ✅ (perma controls)");
  });
  ...
  D.log("[xr] installed ✅ (PERMA + hands visuals)");
      }
