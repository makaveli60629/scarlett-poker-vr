...
(function () {
  ...
  function buildLobby() {
    ...
    world.appendChild(
      el("a-cylinder", {
        ...
        position: "0 5.25 0",
        material:
          "color:#070c12; roughness:0.96; metalness:0.06; side:double; opacity:0.99",
      })
    );
    ...
    world.appendChild(
      el("a-torus", {
        ...
        rotation: "90 0 0",
        material:
          "color:#10314a; emissive:#4aa6ff; emissiveIntensity:0.85; roughness:0.6",
      })
    );

    // Wall trim light ring
    world.appendChild(
      el("a-torus", {
        position: "0 9.9 0",
        radius: "23.15",
        ...
        rotation: "90 0 0",
        material:
          "color:#0b2b44; emissive:#4aa6ff; emissiveIntensity:1.15; opacity:0.92",
      })
    );
    ...
    sign.appendChild(
      el("a-plane", {
        width: "8.2",
        height: "2.4",
        material: "color:#091425; opacity:0.7",
      })
    );
    ...
    // Trophy pedestals (kept around edge)
    for (let i = 0; i < 6; i++) {
      const a = (i / 6) * Math.PI * 2 + 0.3;
      const x = Math.sin(a) * 17.2,
        z = Math.cos(a) * 17.2;
      ...
      ped.appendChild(
        el("a-cylinder", {
          ...
          height: "0.9",
          position: "0 0.45 0",
          material: "color:#0f1a26; roughness:0.75; metalness:0.25",
        })
      );
      ped.appendChild(
        el("a-cone", {
          ...
          height: "0.9",
          position: "0 1.35 0",
          material: "color:#d5b45b; metalness:0.7; roughness:0.35",
        })
      );
  ...
  function buildJumbosAndDoors() {
    // Screens stay where they are; doors + rank plates go UNDER screens.
    const spots = [
    ...
    spots.forEach((s, idx) => {
      ...
      j.appendChild(
        el("a-box", {
          ...
          height: "4.6",
          depth: "0.22",
          material: "color:#0c131d; roughness:0.9",
        })
      );
      j.appendChild(
        el("a-plane", {
      ...
      // Rank plate under jumbotron
      const rank = el("a-entity", {
        position: `${s.x} 5.05 ${s.z}`,
        rotation: `0 ${s.ry} 0`,
      });
      rank.appendChild(
        el("a-plane", {
      ...
      d.appendChild(
        el("a-box", {
          ...
          depth: "0.35",
          position: "0 2.4 0",
          material: "color:#0f1723; roughness:0.9",
        })
      );
      d.appendChild(
        el("a-box", {
          ...
          material: "color:#071018; roughness:1; opacity:0.98",
        })
      );
    ...
    // Store display pedestals (click/gaze to cycle avatars)
    const storeX = -20.5,
      storeZ = 0;
    for (let i = 0; i < 4; i++) {
      const dz = -2.4 + i * 1.6;
      const disp = el("a-entity", {
        class: "storePedestal",
        position: `${storeX + 2.2} 0 ${storeZ + dz}`,
        rotation: "0 90 0",
      });
      disp.appendChild(
        el("a-cylinder", {
          ...
          height: "0.22",
          position: "0 0.11 0",
          material: "color:#0f1a26; roughness:0.7; metalness:0.25",
        })
      );
      ...
      frame.appendChild(
        el("a-plane", {
          width: "0.95",
          height: "1.55",
          material: "color:#061019; opacity:0.55",
        })
      );
  ...
  function buildDivotAndTable() {
    // Divot pit (visual recessed center)
    ...
    pit.appendChild(
      el("a-torus", {
        ...
        rotation: "90 0 0",
        position: "0 0.12 0",
        material: "color:#0b2b44; emissive:#4aa6ff; emissiveIntensity:1.2; opacity:0.95",
      })
    );

    world.appendChild(pit);

    // Table lowered into pit
    ...
    // Markings (simple “engraved” rings/line)
    table.appendChild(
      el("a-ring", {
        rotation: "-90 0 0",
        radiusInner: "1.05",
    ...
    table.appendChild(
      el("a-ring", {
        ...
        radiusOuter: "1.62",
        position: "0 0.90 0",
        material: "color:#e9f2ff; opacity:0.18",
      })
    );
    table.appendChild(
      el("a-plane", {
    ...
    // Community frame + tiny action HUD
    const comm = el("a-entity", { id: "communityFrame", position: "0 1.75 -1.30" });
    comm.appendChild(
      el("a-plane", { width: "2.55", height: "0.86", material: "color:#061019; opacity:0.62" })
    );
    comm.appendChild(
      el("a-plane", {
        width: "2.62",
        height: "0.92",
        position: "0 0 0.01",
        material: "color:#0b2b44; emissive:#4aa6ff; emissiveIntensity:0.35; opacity:0.22",
      })
    );
    ...
    for (let i = 0; i < 5; i++) {
      cards.appendChild(
        el("a-plane", {
          ...
          width: "0.46",
          height: "0.64",
          position: `${(i - 2) * 0.52} -0.08 0`,
          material: "color:#ffffff; opacity:0.12",
        })
      );
    }
    ...
    actionHud.appendChild(
      el("a-plane", { width: "2.35", height: "0.32", material: "color:#091425; opacity:0.72" })
    );
    actionHud.appendChild(
      el("a-plane", {
        width: "2.38",
        height: "0.35",
        position: "0 0 0.01",
        material: "color:#4aa6ff; opacity:0.08; emissive:#4aa6ff; emissiveIntensity:0.35",
      })
    );
    actionHud.appendChild(
      el("a-text", {
    ...
    pot.appendChild(
      el("a-text", { id: "potText", value: "POT $0", position: "0 0 0.01", align: "center", width: "3.4", color: "#d7eaff" })
    );
    ...
    for (let i = 0; i < 10; i++) {
      chipPile.appendChild(
        el("a-cylinder", {
          radius: "0.06",
          height: "0.02",
          position: `${(Math.random() * 0.30 - 0.15).toFixed(2)} ${(i * 0.02).toFixed(2)} ${(Math.random() * 0.30 - 0.15).toFixed(2)}`,
          material: "color:#e9f2ff; roughness:0.6; metalness:0.1",
        })
      );
    }
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const x = Math.sin(ang) * 3.85,
        z = Math.cos(ang) * 3.85;
      const yaw = (Math.atan2(x, z) * 180) / Math.PI + 180;

      const chair = el("a-entity", {
        class: "chair",
        position: `${x.toFixed(2)} 0 ${z.toFixed(2)}`,
        rotation: `0 ${yaw.toFixed(1)} 0`,
      });
    ...
    for (let i = 0; i < 4; i++) {
      stairs.appendChild(
        el("a-box", {
          ...
          height: "0.18",
          depth: "0.65",
          position: `0 ${(0.18 * i).toFixed(2)} ${(-i * 0.65).toFixed(2)}`,
          material: "color:#0b1119; roughness:1",
        })
      );
    }
  ...
  function buildBots() {
    ...
    for (let i = 0; i < 6; i++) {
      const ang = (i / 6) * Math.PI * 2;
      const x = Math.sin(ang) * 3.55,
        z = Math.cos(ang) * 3.55;
      const yaw = (Math.atan2(x, z) * 180) / Math.PI + 180;

      const bot = el("a-entity", {
        class: "bot",
        "data-seat": String(i + 1),
        position: `${x.toFixed(2)} -0.85 ${z.toFixed(2)}`,
        rotation: `0 ${yaw.toFixed(1)} 0`,
      });

      // Placeholder body (avatar_loader swaps these if GLBs load)
      ...
      // Name tag (hidden; focus script toggles)
  ...
  function build() {
    if (!world) {
      D && D.log && D.log("[world] ERROR: #world entity missing");
      return;
    }
    clear();
    buildLobby();
    ...
    buildDivotAndTable();
    buildBots();
    D && D.log && D.log("[world] build ✅");
  }

  // ✅ CRITICAL FIX: always expose SCARLETT_WORLD.build immediately
  window.SCARLETT_WORLD = window.SCARLETT_WORLD || {};
  window.SCARLETT_WORLD.build = build;
})();
