// /js/world.js — DIAGNOSTIC WORLD (FULL) Update 4.8.6
export const World = (() => {
  ...
  function buildRoomShell({ THREE, group, center, size=10, h=4.0, matWall, matFloor, doorSide }) {
    ...
    // Skip the wall facing the lobby -> never boxed
  ...
  function build({ THREE, scene, renderer, camera, player, controllers, world, tag, BUILD }) {
    tag?.("world", `LOADED world.js Update 4.8.6 ✅`);
    ...
    for (let i=0;i<roomCenters.length;i++){
      buildRoomShell({ THREE, group: world.group, center: roomCenters[i], size: ROOM_SIZE, h: ROOM_H, matWall, matFloor, doorSide: doorSides[i] });
    }
    ...
    for (let i=0;i<roomCenters.length;i++){
      ...
      buildHallway({ THREE, group: world.group, from, to, w: HALL_W, h: HALL_H, matWall, matFloor });
    }

    tag?.("world", "Update 4.8.6 built ✅ (FULL DIAG + HALLWAYS)");
  }
