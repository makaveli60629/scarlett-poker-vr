...
// ✅ If your forward/back is still reversed on your Quest, flip this to false.
const QUEST_FORWARD_IS_NEGATIVE_Y = true;
...
function setupXRControls() {
  ...
  // laser pointer on right hand grip
...
function applyLocomotion(dt) {
  ...
  if (gpL?.axes?.length >= 2) {
    ...
    if (ax || ay) {
      ...
      const forwardScalar = QUEST_FORWARD_IS_NEGATIVE_Y ? (-ay) : (ay);

      const move = new THREE.Vector3();
      move.addScaledVector(forward, (forwardScalar) * MOVE_SPEED * dt);
