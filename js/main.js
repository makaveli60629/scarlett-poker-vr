...
function buildControllers() {
  ...
  // IMPORTANT: Parent to rig so they NEVER drift away from you
  rig.add(controller0);
  rig.add(controller1);
  ...
  // Right-hand rainbow ray (forced downward in updateRay())
...
function updateRayAndReticle() {
  // Always show reticle. If not in XR, park it in front of you.
  ...
  // Build a ray direction that is ALWAYS tilted downward (so you don’t have to point to the floor)
  ...
  // Force downward pitch ~25 degrees minimum
  const down = new THREE.Vector3(dir.x, -0.55, dir.z).normalize();
  dir = down;
  ...
  // Make ray length match intersection
  const dist = origin.distanceTo(target);
  rayLine.scale.z = clamp(dist, 0.5, 18);

  // Tiny rainbow shimmer by cycling endpoint color
  const geom = rayLine.geometry;
  const col = geom.getAttribute("color");
  const t = clock.getElapsedTime();
  col.setXYZ(1, 0.5 + 0.5*Math.sin(t*2.2), 0.5 + 0.5*Math.sin(t*2.2 + 2.1), 0.5 + 0.5*Math.sin(t*2.2 + 4.2));
