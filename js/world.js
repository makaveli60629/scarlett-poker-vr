// /js/world.js — Scarlett MASTER WORLD v4 (PokerSim integrated)
// ✅ Keeps your wall structure concept (solid ring wall + 4 door gaps)
// ✅ Pit + table + rail + stairs + bots + chips
// ✅ Real poker dealing via PokerSim (no scattered demo cards)
// ✅ GitHub Pages safe: no bare imports like "three"
...
export const World = (() => {
  ...
  function buildOuterWallsPreserved() {
    ...
    function makeSegment(a0, a1) {
      ...
      const wall = new THREE.Mesh(geo, mat(0x0b0d14, 0.95, 0.06, 0x000000, 0, S.tex));
      wall.position.y = 0;
      add(wall, true);
  ...
  function buildPitTableRailStairs() {
    ...
    // Rail with opening + gate + short stairs
  ...
  function buildBots() {
    ...
    function displayCase(angle, labelColor) {
      ...
      const caseG = new S.THREE.Group();
      caseG.position.set(x, 0, z);
      caseG.rotation.y = rotY;

      const base = new S.THREE.Mesh(new S.THREE.BoxGeometry(2.4, 0.25, 1.2), mat(0x111326, 0.9, 0.2, labelColor, 0.15));
      base.position.y = 0.125;
      caseG.add(base);

      const glass = new S.THREE.Mesh(
        new S.THREE.BoxGeometry(2.2, 2.4, 1.0),
        new S.THREE.MeshStandardMaterial({ color: 0x7fe7ff, roughness: 0.1, metalness: 0.05, transparent: true, opacity: 0.12 })
      );
  ...
  function buildChips() {
    const chipGeo = new S.THREE.CylinderGeometry(0.09, 0.09, 0.035, 18);
    for (let i = 0; i < 18; i++) {
      const chip = new S.THREE.Mesh(chipGeo, mat(i % 2 ? 0xff2d7a : 0x7fe7ff, 0.6, 0.15));
      add(chip);
      S.chips.push(chip);
    }
    log("[world] chips ✅");
  ...
  async function init({ THREE, scene, renderer, camera, player, controllers, log: logFn }) {
    ...
    log("[world] build complete ✅ (MASTER v4)");
  }
