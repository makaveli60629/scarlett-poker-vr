// /js/world.js — Scarlett MASTER WORLD v11.1 (FIX NULL VISIBLE CRASH + STABLE ORDER)
// ✅ Fixes: Cannot set properties of null (setting 'visible') by creating crown/banner before poker
// ✅ Adds null guards so it can never crash again
// ✅ Keeps: head cards billboard + readable, higher community, shallower pit, knees/ankles, winner banner+crown
...
export const World = (() => {
  const S = {
    ...
    bots: { lobbyWalkers: [], pitPlayers: [], pitPlayersGroup: null },

    poker: {
      seats: 6, seatPos: [], stage: "idle", stageT: 0, activeSeat: 0,
      pot: 0, bet: 0, action: "CHECK", winnerSeat: -1, winnerName: "",
      tableCardsRoot: null, community: [],
      headCardsRoot: null, headCards: [],
      deck: [],
    },
    ...
    chips: {
      root: null, pool: [], fly: [], stacked: [],
      seatTargets: [], potCenter: null, lastTurnThrowAt: 0,
    },

    crown: { mesh: null, seat: -1, t: 0, active: false },
    winnerBanner: { root: null, tag: null, active: false, t: 0 },
  ...
  const addCollider = (o)=>{ if(o) S.colliders.push(o); };
  ...
  function makeMarbleTexture(){
    ...
    g.fillStyle="#1a1f2a"; g.fillRect(0,0,c.width,c.height);

    for(let i=0;i<18000;i++){
      const x=Math.random()*c.width, y=Math.random()*c.height;
      const r=Math.random()*2.2, a=0.04+Math.random()*0.07;
  ...
  function buildLights(){
    ...
    dir.position.set(8,18,10); S.scene.add(dir);
    ...
    tableGlow.position.set(0,3.2,0); S.scene.add(tableGlow);
  ...
  function buildLobbyShell(){
    ...
    S.root.add(G); S.lobby = G;

    const floorMat = new S.THREE.MeshStandardMaterial({
      map: S.marbleTex, roughness:0.9, metalness:0.05,
      color: new S.THREE.Color(0xffffff),
    });
    const floor = new S.THREE.Mesh(new S.THREE.CircleGeometry(26,140), floorMat);
    floor.rotation.x = -Math.PI/2;
    G.add(floor); addCollider(floor);

    const wallMat = new S.THREE.MeshStandardMaterial({
      color:new S.THREE.Color(0x6b7078), roughness:1, metalness:0.05, side:S.THREE.DoubleSide
    });
    const wall = new S.THREE.Mesh(new S.THREE.CylinderGeometry(26,26,20,160,1,true), wallMat);
    wall.position.y = 10; G.add(wall);

    const ring = new S.THREE.Mesh(new S.THREE.TorusGeometry(25.5,0.10,16,180), stdMat(0x2a2f52,0.8,0.2,0x7fe7ff,0.55));
    ring.position.y = 19.2; ring.rotation.x = Math.PI/2; G.add(ring);
  }
  ...
  function buildPitSystem(){
    ...
    S.lobby.add(pitG); S.pitG = pitG;

    const ring = new S.THREE.Mesh(new S.THREE.RingGeometry(radiusInner+0.08, radiusOuter, 160), stdMat(0x1a2030,1,0.06));
    ring.rotation.x = -Math.PI/2; pitG.add(ring); addCollider(ring);

    const pitFloor = new S.THREE.Mesh(new S.THREE.CircleGeometry(radiusInner,160), stdMat(0x0a0c16,1,0.06));
    pitFloor.rotation.x = -Math.PI/2; pitFloor.position.y = -depth;
    pitG.add(pitFloor); addCollider(pitFloor);

    const pitWall = new S.THREE.Mesh(
      new S.THREE.CylinderGeometry(radiusInner, radiusInner, depth, 160, 1, true),
      new S.THREE.MeshStandardMaterial({ color:new S.THREE.Color(0x3a3f48), roughness:1, metalness:0.04, side:S.THREE.DoubleSide })
    );
    pitWall.position.y = -depth/2; pitG.add(pitWall);

    const trim = new S.THREE.Mesh(new S.THREE.TorusGeometry(radiusInner+0.06,0.10,18,180), stdMat(0x1b2242,0.65,0.2,0x7fe7ff,0.35));
    trim.position.y = 0.06; trim.rotation.x = Math.PI/2; pitG.add(trim);

    const railRadius = radiusInner + 0.24;
    const rail1 = new S.THREE.Mesh(new S.THREE.TorusGeometry(railRadius,0.06,18,180), stdMat(0x2b2f52,0.7,0.25,0x7fe7ff,0.18));
    rail1.position.y = 1.10; rail1.rotation.x = Math.PI/2; pitG.add(rail1);
    const rail2 = new S.THREE.Mesh(new S.THREE.TorusGeometry(railRadius,0.04,18,180), stdMat(0x2b2f52,0.75,0.25,0xb56bff,0.14));
    rail2.position.y = 0.72; rail2.rotation.x = Math.PI/2; pitG.add(rail2);
    ...
    for(let i=0;i<6;i++){
      ...
      chairG.add(makeChair(Math.cos(a)*seatR, Math.sin(a)*seatR, -a + Math.PI));
    }

    S.chips.potCenter = v3(0, 1.18, -0.10);
  }

  function makeChair(x=0,z=0,ry=0){
    const G = new S.THREE.Group();
    ...
    back.position.set(0,0.85,-0.30); G.add(back);
  ...
  function buildJumbotrons(){
    ...
    const r=25.85, y=11.2;
    for(let i=0;i<4;i++){
      const a=(i/4)*Math.PI*2 + Math.PI/4;
      const frame = new S.THREE.Mesh(frameGeo, frameMat);
      frame.position.set(Math.cos(a)*r,y,Math.sin(a)*r);
      frame.lookAt(0,y,0);
  ...
  function makeBigHUD(lines){
    ...
    const draw=(l1,l2)=>{
      ...
      g.fillStyle="rgba(10,12,18,0.42)"; g.fillRect(0,0,c.width,c.height);
      g.strokeStyle="rgba(255,255,255,0.22)"; g.lineWidth=8;
  ...
  // ----- Winner banner + Crown (built BEFORE poker now) -----
  ...
  function startWinnerBanner(name){
    if(!S.winnerBanner.root || !S.winnerBanner.tag) return;
  ...
  function makeHeadCard(){
    ...
    const mat=new S.THREE.MeshBasicMaterial({ color:0xffffff, transparent:true, opacity:1, side:S.THREE.DoubleSide });
    return new S.THREE.Mesh(geo,mat);
  }

  function initPoker(){
    const P=S.poker;
    P.stage="preflop"; P.stageT=0; P.activeSeat=0;
    P.pot=0; P.bet=0; P.action="CHECK";
    P.winnerSeat=-1; P.winnerName="";
    buildDeck();
    ...
    for(let i=0;i<5;i++){
      ...
      c.position.set(-1.7 + i*0.90, 2.05, 0);
      c.visible=false;
  ...
  function newHand(){
    ...
    // pre-generate community textures (shown later)
    for(let i=0;i<5;i++){
      const card=drawCardFromDeck();
      applyCardTexture(P.community[i], card.r, card.s);
    }

    // ✅ null-safe reset visuals
    if(S.crown.mesh){
      S.crown.active=false; S.crown.seat=-1; S.crown.mesh.visible=false;
    }
    if(S.winnerBanner.root){
      S.winnerBanner.active=false; S.winnerBanner.root.visible=false;
    }
  ...
  function stepPoker(dt){
    ...
    } else if(P.stage==="flop" && P.stageT>CADENCE.turn){
      P.community[3].visible=true;
      P.stage="turn"; P.stageT=0;
    } else if(P.stage==="turn" && P.stageT>CADENCE.river){
      P.community[4].visible=true;
      P.stage="river"; P.stageT=0;
    } else if(P.stage==="river" && P.stageT>CADENCE.complete){
      P.stage="showdown"; P.stageT=0;
    } else if(P.stage==="showdown" && P.stageT>CADENCE.showdown){
      doShowdown();
      P.stage="idle"; P.stageT=0;
      setTimeout(()=>newHand(), 2600);
    }
  }
  ...
  function getFreeChip(){ return S.chips.pool.find(c=>!c.visible)||null; }

  function potStackPos(){
    const base=S.chips.potCenter.clone();
    const n=S.chips.stacked.length;
    const jitter=0.16;
    return base.add(v3((Math.random()-0.5)*jitter, 0.03*(n%12), (Math.random()-0.5)*jitter));
  }

  function throwChip(from,to,landToStack=true,landHide=false){
    const chip=getFreeChip(); if(!chip) return;
    chip.visible=true;
    chip.position.copy(from);
    chip.rotation.set(0,0,0);
    S.chips.fly.push({
      chip,t:0,dur:0.42+Math.random()*0.20,
      from:from.clone(), to:to.clone(),
      arc:0.34+Math.random()*0.26,
      landToStack, landHide
    });
  }

  function vacuumPotToWinner(seat){
    const to=S.chips.seatTargets[seat]?.clone()||v3(0,1.2,3);
  ...
  // bots (keep whatever you already had in index; this world only animates them visually)
  function makeTextTag(text){
    const c=document.createElement("canvas");
    c.width=512; c.height=128;
    const g=c.getContext("2d");
    g.fillStyle="rgba(10,12,18,0.35)"; g.fillRect(0,0,c.width,c.height);
  ...
  function makeBot(name="BOT", color=0x2a2f52, accent=0x7fe7ff){
    ...
    shoulder.position.y=1.62; G.add(shoulder);

    const head=new S.THREE.Mesh(new S.THREE.SphereGeometry(0.22,18,18), trim);
    head.position.y=1.86; head.userData._head=true; G.add(head);
    ...
    tag.position.set(0,2.38,0); tag.scale.setScalar(0.42);
    G.add(tag);
  ...
  function updateBots(dt){
    ...
    for(const b of S.bots.lobbyWalkers){
      ...
      faceYawOnly(b, v3(Math.cos(a+0.02)*R,0,Math.sin(a+0.02)*R));
  ...
  async function init({ THREE, scene, renderer, camera, player, controllers, log }){
    ...
    S.player=player; S.controllers=controllers; S.log=log||console.log;
    ...
    // ✅ FIX: build crown/banner BEFORE poker (so newHand can safely touch them)
    buildCrown();
    buildWinnerBanner();
    ...
    S.log("[world] build complete ✅ (MASTER v11.1)");
  }
