...
function floorTex(){
  return canvasTex((ctx,w,h)=>{
    ...
    const g=ctx.createRadialGradient(w/2,h/2,80,w/2,h/2,290);
...
export async function boot(){
  ...
  const stripM=new THREE.MeshStandardMaterial({color:0x111219, emissive:0x00ffd5, emissiveIntensity:0.75, roughness:0.25, metalness:0.1});
  const strip=(x,z,rot)=>{
  ...
  // Audio toggle (works if file exists)
  ...
  if($('btnMenu')) $('btnMenu').onclick=()=>{ logLine('⌚ Menu (wrist menu next step)'); };

  const joy = bindJoystick();

  // Poker loop state machine
  ...
  function newHand(){
    ...
    // Hole cards: seat 0 face-up, others face-down
    for(let i=0;i<seatCount;i++){
      const faceUp = (i===0);
  ...
  function showdown(){
    const winner=(Math.random()*CFG.seatedBots)|0; // pick among seated bots
  ...
  renderer.xr.addEventListener('sessionstart', ()=>{ renderer.setPixelRatio(1); logLine('✅ XR session start'); });
  ...
  renderer.setAnimationLoop(()=>{
    ...
    // movement (Android joystick + VR stick will be added next)
  ...
  logLine('✅ Full build loaded: oval table + bots + poker loop + textures');
}

/* ---- Self-run fallback (if someone opens main.js without loader) ---- */
if (typeof window !== 'undefined') {
  // Only run once
  if (!window.__NOVA_MAIN_STARTED__) {
    window.__NOVA_MAIN_STARTED__ = true;
    // If loader calls boot(), it will run anyway; this just prevents “nothing happens” scenarios.
    // Delay slightly so loader can import first.
    setTimeout(() => {
      // If status is still “loading…”, start boot
