// js/main.js — Patch 6.5 FULL
// Adds: Interactions (raycast + grip pickup) + kiosk open via grip
// Keeps: Patch 6.4 Shop + Inventory + AvatarShop + Patch 6.3 collision/walls
...
export async function boot() {
  ...
  // Build kiosk + chip, capture their objects for Interactions
  ...
  overlay("Loaded ✅\nPatch 6.5: Grip Interactions + Pickup + Kiosk Toggle");
  ...
  renderer.setAnimationLoop(() => {
    ...
    // Menu toggle still works (and M on phone)
    if (Input.menuPressed()) {
      ShopUI.toggle();
      toast(ShopUI.open ? "Store opened" : "Store closed");
    }

    // Grip now does real actions (kiosk/chip)
